...
def main():
    ...
    # v3:同團(gggg)成員同艙同命運 → 隨機切跨折互漏(v2 OOF +.010 但 LB −.0007,C7b 第三例)→ GroupKFold
    folds = list(StratifiedGroupKFold(5, shuffle=True, random_state=SEED).split(tr, y, groups=tr["grp"]))
    ...
    pd.DataFrame({"PassengerId": te["PassengerId"], "Transported": (pick_te > th)}).to_csv(
        Path(__file__).parent / "submission_spaceship_v3.csv", index=False)
