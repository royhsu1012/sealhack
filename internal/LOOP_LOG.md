...
### 全站縮寫降噪:CV/OOF/LB/AUC/L1-L3 中文配對(使用者直接指令)|2026-09-13
承前一輪 Kaggle+LeetCode 改版後,使用者再指令「太多專有名詞 會看不懂 用 不要用簡寫」。
用 AskUserQuestion 確認範圍(具體表格標題 vs 全站核心縮寫)——使用者選「全站核心縮寫(CV/OOF/LB/AUC 等)都要改」,
這與 CLAUDE.md「不改寫技術內容」有張力,已在問題裡明確點出後由使用者拍板才動手。

**規模盤點**:CV 全站 135 次、OOF 76、LB 59、AUC 41(prose 內)。若逐字取代會讓句子腫脹不可讀,
改用技術寫作慣例:**每個 ## 段落內,該縮寫第一次裸出現時配對「中文全稱(縮寫)」,段落內後續維持裸寫**——
既解決「看不懂」又不犧牲可讀性,不違反內容保留鐵律。

**做法**:兩輪 Workflow(共 12 頁 × 2,部分重疊)。第一輪處理 CV/OOF/LB/AUC/RMSE/MAE/GBDT/KNN/SVM/PCA(11 頁平行,11/11 成功)。
複核 diff 時發現自己漏了使用者最初舉的例子——L1/L2/L3(方法論驗證等級代碼)——追加第二輪(7 頁平行,7/7 成功)。
ClaimsTable/CasesTable 表格標題(L1/L2/L3、CV/holdout/public LB)由主 session 直接改,不進 Workflow。

**品質驗證(不盡信 agent 摘要)**:逐檔 Read 全部 13 個檔案的完整 diff;grep 確認無雙重配對(如「(CV)(CV)」)、
無配對文字誤入 code fence(寫 python 腳本掃全部 fence 內容驗證)、零 ##/### 標題被改動(錨點 slug 完整保留)、
無孤兒連結(href 全部保留原樣,只動了顯示文字)。build 13 頁零錯、check.py S6/S10=0。

**意外發現**:quickstart.mdx 的 agent 正確**拒絕**套用配對——該頁的 L1/L2/L3 其實是完全不同的系統(學習地圖 L0-L4 難度分級:
L1=心智核心、L2=表格骨幹…),硬套 claims.json 的「L1=文獻收斂」會寫出語意錯誤的句子。已核實(對照 claims.json)並保留原樣。
**待辦(未處理,留意但不緊急)**:網站同時存在兩套撞名的 L1/L2/L3 系統(驗證等級 vs 學習難度),對讀者可能仍是潛在混淆源,
但屬於命名設計決策,不在本輪「加中文配對」範圍內,若要處理需使用者決定其中一套改名。

自省:**Workflow agent 的「拒絕執行」比「盲目執行」更有價值**——這次證明分派任務時給清楚的正確性檢查規則
(不確定就對照資料源頭、寧可跳過不要猜)比要求 100% 完成率更重要,agent 抓到了我自己在寫規格時漏想的語意衝突。
