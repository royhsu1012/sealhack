...
> 路線:[🐣](/workflow/handbook/) → [0](/workflow/0-diagnose/) → **1 驗證** → [2](/workflow/2-baseline/) → [3](/workflow/3-features/) → [4–5](/workflow/4-ensemble/) ｜ ⏱ 約 16 分 · 前置:階段 0
>
> 先做出一把**可信的尺**(驗證方法),之後每個改動都用它量。有第一次成功提交之前,不要碰特徵工程。

**你要做的三件事**

1. 24 小時內跑通「讀檔 → 模型 → 交卷」整條路
2. 選對切法,讓分數不會騙你
3. 把 OOF 存檔——後面全靠它
...
| 術語 | 白話 |
|---|---|
| **OOF** | 每筆訓練資料「被當成考題」時得到的預測,拼成完整一份;它是誠實的成績單,也是後面組合模型的原料 |
| **折(fold)** | 把資料切成 K 份,每一份就是一折 |
| **端到端** | 從讀檔案到交出提交檔,整條流程跑通一次 |

</details>

## 階段 1:鎖死 CV + 端到端提交(1 天)
...
1. 為什麼「有第一次成功提交之前,不要碰特徵工程」?
2. OOF 是什麼?為什麼能拿它組合模型,訓練集上的預測卻不行?
3. 你的資料裡同一個使用者出現很多次,該用哪種切法?

<details>
<summary><strong>參考答案(先自己想過再展開)</strong></summary>

1. 因為沒跑通端到端就不知道格式、指標、時間成本對不對;先拿到一個真實分數當地基,後面每個改動才有東西可比。

2. OOF = 每筆資料「被當成考題」時、由沒看過它的模型給的預測。訓練集預測是模型看過答案後的自我評分,拿去組合會嚴重高估(C6:in-sample 表面 1.0000 → 私榜 0.7760)。
