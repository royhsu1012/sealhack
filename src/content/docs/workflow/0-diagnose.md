...
> 路線:[🐣](/workflow/handbook/) → **0 診斷** → [1](/workflow/1-validate/) → [2](/workflow/2-baseline/) → [3](/workflow/3-features/) → [4–5](/workflow/4-ensemble/) ｜ ⏱ 約 17 分 · 前置:做過一次手冊
>
...
2. **謹慎驗證**:交叉驗證(CV) 切法必須對應 test 的結構。CV 不可信 = 後面每個決策都是賭博。
...
### 必須回答的問題(Data Memo 模板)

| 專案 | 內容 |
|---|---|
| 任務型別 | 二分類 / 多分類 / 迴歸 / 排序 |
| 評估指標 | 曲線下面積(AUC) / LogLoss / 均方根誤差(RMSE) / 平均絕對誤差(MAE) / MAP@k / 自訂 |
...
| Public 排行榜(LB) 佔比 | 決定你能多信 LB |

### 指標決定的戰術差異

| 指標 | 關鍵動作 |
|---|---|
| AUC | 只在乎排序;不需要校準。整合預設用機率平均,**成員尺度差異大時才轉 rank**(見集成頁 L2(沙盒實驗) 實驗) |
...
| F1 / Accuracy | **閾值搜尋是必做項**,在折外預測(OOF)上搜最優閾值 |

### 對抗驗證 Adversarial Validation(強烈建議第一天就跑)
...
## 階段 0.5|資料清洗與補全(v2.2 新增,補結構性缺口)

位置:階段 0 診斷之後、階段 1 鎖 交叉驗證(CV) 之前做初版;之後每個清洗選擇都回到實驗迴圈。
...
1. **fold 內擬合所有統計**。分級對待:不碰目標的統計(中位數/眾數/縮放)
   洩漏是二階小量(L2(沙盒實驗) 實測 |Δ|≈0.0001,small_n_paired.py 20 切分),fold 內做是衛生習慣;
...
對抗驗證不是儀式:s6e8(69 萬列 synthetic)開跑前先測 train-vs-test 可分性得 **曲線下面積(AUC) 0.553 ≈ 0.5** → 診斷「隨機切」成立;之後四筆真提交的 交叉驗證(CV)→排行榜(LB) 落差全部只有 **+0.0013~0.0014**——切法對,CV 就誠實([`case_s6e8.py`](/validation/case_s6e8.py))。對照組:鐵達尼真實 test 為異分布,同一套流程 CV−LB 差 0.06~0.09。**先測分布,再選切法,落差可預測。**
...
2. 不填。LightGBM 原生處理 NaN,填補反而抹掉「缺失本身就是訊號」;只加一欄 `is_missing` 記錄。線性/K 近鄰(KNN)/神經網路才必填。

3. 指標決定所有戰術:曲線下面積(AUC) 只看排序(不必校準機率)、LogLoss 在乎機率準不準、Accuracy/F1 要在折外預測(OOF)上搜最佳閾值。答不出來就回頭讀題。
...
- [`case_s6e8.py`](/validation/case_s6e8.py) —— 開跑前先做對抗驗證:印出 train/test 可分性 曲線下面積(AUC)。0.553≈0.5 → 診斷「隨機切」成立,之後四筆真提交的 交叉驗證(CV)−排行榜(LB) 落差果然只有 ±0.0014。
